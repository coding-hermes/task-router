...
|---|---|---|
| pi | some OpenAI-compatible servers reject `role:developer`; pi exposes `compat.supportsDeveloperRole=false` | **DELIVERED (TR-074):** the driver declares `supportsDeveloperRole=false` on the route; the proxy independently normalizes a `developer` role that still arrives, and discloses the rewrite |
| deepseek-harness | hand-declared routes default to OpenAI-own shape (`supportsDeveloperRole=true`, `maxTokensField=max_completion_tokens`, `supportsStore=true`) | **DELIVERED (TR-075):** the driver declares all THREE — `supportsDeveloperRole=false`, `maxTokensField='max_tokens'`, `supportsStore=false`. `supportsDeveloperRole` alone is insufficient: `maxTokensField` shapes EVERY request, so it carries the same defect over a wider blast radius. Confirmed in shipped source (`PiAiCompatProfile`, 26 fields) and against the harness's own offered-field gate, which REFUSES an unknown key at write time rather than dropping it |
| opencode | accepts custom OpenAI-compatible providers AND speaks ACP; **[VERIFIED 2026-09-20]** uses `@ai-sdk/openai-compatible` for a hand-declared provider, so it sends **openai-chat**, not `anthropic-messages` (119/119 requests to `/v1/chat/completions`, zero anthropic markers) | W for the proxy path; A is only for fleet-spawned runs. The dialect MUST be declared (`npm: '@ai-sdk/openai-compatible'`) — `api.id` is an open string, so omit-vs-omit is not safe. Attribution needs `options.headers['x-router-caller']` AND the driver registered |
