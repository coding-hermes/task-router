...
class RouterApplication:
    ...
    def dispatch(self, method, path, query=None, body=None, headers=None):
        ...
        if method == "GET":
            ...
            if path == "/health":
                payload = router_health.health(mode=self.mode)
                # TR-172: the admission bound and the rating cache are facts about THIS
                # process, so /health carries them. An operator has to be able to see
                # that the proxy is refusing work BEFORE a caller reports it, and a cap
                # nobody can read is a cap nobody can tune.
...
def _proxy_record(provider, model, ok, requirements, reason='', latency_s=None,
    ...
    try:
        ...
        # Router-internal outcomes describe admission/degradation, not a provider
        # failure. Sending them to the provider-wide breaker recreates the incident
        # loop: the proxy's own 429/no-hop response would open every provider.
